...
export const OPEN_MODE_VALUES = ["popup", "tab"];
...
export const AI_PROVIDER_VALUES = [
  ...
  "claude-ai",
  "openai-codex",
  "salesforce-einstein-llm",
  "salesforce-models-api",
  "agentforce-agent",
  "gemini-api", // Legacy mode kept for backward compatibility.
];
...
export function normalizeOpenMode(raw) {
  const mode = String(raw || "").toLowerCase();
  return OPEN_MODE_VALUES.includes(mode)
    ? mode
    : SETTINGS_STORAGE_DEFAULTS.openMode;
}

export function normalizeAiProvider(raw) {
  const legacyAliasMap = {
    "openai-compatible": "openai-codex",
    agentforce: "agentforce-agent",
    "anthropic-claude": "claude-ai",
  };
  const provider = String(raw || "").toLowerCase();
  const normalized = legacyAliasMap[provider] || provider;
  return AI_PROVIDER_VALUES.includes(provider)
    ? provider
    : AI_PROVIDER_VALUES.includes(normalized)
      ? normalized
    : SETTINGS_STORAGE_DEFAULTS.aiProvider;
}

export function sanitizeAiText(raw, fallback = "") {
