 ...
 * @param {boolean} emphasizeExceptions - bright red rows when true (exceptions-only filter)
 * @param {() => string} [assignErrorRowId] - returns stable id for each exception row (scroll targets)
 */
function renderLogDataTableHtml(lines, emphasizeExceptions, assignErrorRowId) {
  const rows = lines
    .map((row) => {
      ...
      const trClass =
        emphasizeExceptions && err
          ? "log-data-row log-data-row--error"
          : "log-data-row";
      const typeCell = err
        ? `<span class="log-type-cell"><code>${type}</code><span class="log-row-badge" title="Error or exception">Error</span></span>`
        : `<code>${type}</code>`;
      const idAttr =
        err && typeof assignErrorRowId === "function"
          ? ` id="${escapeHtml(assignErrorRowId())}"`
          : "";
